...
Add the **360 LLM-generated** sentences to each fraction.
...
| Parameters | 109.5M | 67.0M | **23.9M**^[This counts only the parameters still stored as fp32 (the embeddings) — dynamic int8 quantizes the `Linear` layers, whose weights are no longer counted as parameters. The model still holds the same ~110M weights; quantization changes their precision, not their count.] |
...
- **Feature-based distillation** — transfer hidden states, not just logits (e.g. TinyBERT).
- **More transfer data** — teacher soft-labels the LLM-generated / unlabelled text.
- **Tune T and α** — push more of the teacher's soft signal to the student to protect the positive-vs-neutral boundary.
- **Combine techniques** - distil for speed, quantize to int8 for memory, and QLoRA to fine-tune under a tight memory budget.
:::
:::
